...
**BEFORE pasting the prompt:** invoke the `onboarding-planner` skill in your Claude.ai sub-chat for this app. The skill runs a 16-question interview about the app, target audience, and core problem, then generates a screen-by-screen plan using Mau Baron's $40k/month framework as the master spine. Drop the resulting `onboarding-plan.md` at `app-references/onboarding-plan.md` in the repo before pasting the Stage 4 prompt below.

If `app-references/onboarding-plan.md` does not exist when you paste this prompt, the agent will stop. The plan is the spec — don't improvise.

```
SKILLS: Use screen-wiring and building-native-ui skills for this work.

Read CLAUDE.md, ONBOARDING_PATTERNS.md, and app-references/onboarding-plan.md.
Also read the existing onboarding screens in src/screens/onboarding.

If app-references/onboarding-plan.md does NOT exist, STOP and tell me. The
plan is generated by the onboarding-planner skill in a Claude.ai planning
sub-chat BEFORE this stage. Do not improvise the plan.

The plan is the single source of truth for what to build. Each screen entry
specifies: Goal, Copy draft, Visual spec, Pattern reference (from
ONBOARDING_PATTERNS.md), and Mau parallel. The plan applies Mau Baron's
3-act, ~30-screen framework (Introduction → Climax → Conclusion) to this
app's specific audience.

If the plan has open questions or "unsure" items at the bottom, STOP and tell
me before starting — those need answers first.

---

Your job: build the onboarding from the plan, exactly as specified.
...
- Quiz answers persist to AsyncStorage under 'onboarding_quiz' key and are
  read by downstream personalization screens (Reflection, Summary, Faith
  Snapshot equivalents) to inject user-specific content.
- Review modal fires at the climax screen specified in the plan (typically
  screen 24 in the Mau structure) — NOT at the end of the flow. This is
  Mau's 1-in-8 review rate trick; emotional peak placement matters.
- Paywall variant(s) per the plan's paywall section: all have VISIBLE skip
  link. Real payment hookup goes to LATER.md.
...
Phase 4.1 — Validate the plan
Read app-references/onboarding-plan.md in full. Confirm:
- Every screen has Goal, Copy, Visual, Pattern, and Mau parallel
- Pattern numbers reference real entries in ONBOARDING_PATTERNS.md
- Total screen count is in the 25–30 range
- Review modal placement is at the emotional peak (NOT the end)
- "Open questions" section at the bottom is empty or already addressed
List anything ambiguous or that you'd flag as a problem before starting.
Do NOT write code yet. Wait for my OK.

Phase 4.2 — Quiz screen + data layer
Build the question bank screens (typically screens 4–17 in the Mau structure)
plus the AsyncStorage data layer FIRST. Every downstream screen depends on
quiz data for the reflection/summary/snapshot screens.
→ Run /skill self-check. STOP for approval.

Phase 4.3 — Build remaining screens, ONE AT A TIME
For each screen in the plan, in order:
...
Phase 4.4 — Paywall variant(s) + skip logic + review modal trigger
Build the paywall variant(s) per the plan's paywall section. Wire the native
iOS review prompt to fire at the climax screen specified in the plan
(use expo-store-review's StoreReview.requestReview()). Wire skip logic:
...
- What I should see: <first screen name from plan>
- Total screen count in the flow
- Which screens read from quiz data for personalization
- Confirmation review modal fires at the climax screen, NOT at the end

Terminal commands you produce must assume the terminal is already in the
repo root — never prefix with cd <repo>. cd is only acceptable to move into
a subdirectory of this repo.
