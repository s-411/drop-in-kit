...
This declares what providers are enabled and exports the auth handlers. Start with anonymous; add others as needed.
...
`Anonymous` is a named import from `@convex-dev/auth/providers/Anonymous` (capital `A` in the path). Do not import from `@auth/core` — that's the Auth.js core lib, not Convex Auth's provider wrapper.
